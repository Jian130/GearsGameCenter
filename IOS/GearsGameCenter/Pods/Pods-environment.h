
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// BLWebSocketsServer
#define COCOAPODS_POD_AVAILABLE_BLWebSocketsServer
#define COCOAPODS_VERSION_MAJOR_BLWebSocketsServer 0
#define COCOAPODS_VERSION_MINOR_BLWebSocketsServer 1
#define COCOAPODS_VERSION_PATCH_BLWebSocketsServer 0

// GCDWebServer
#define COCOAPODS_POD_AVAILABLE_GCDWebServer
#define COCOAPODS_VERSION_MAJOR_GCDWebServer 1
#define COCOAPODS_VERSION_MINOR_GCDWebServer 2
#define COCOAPODS_VERSION_PATCH_GCDWebServer 2

// JSONModel
#define COCOAPODS_POD_AVAILABLE_JSONModel
#define COCOAPODS_VERSION_MAJOR_JSONModel 0
#define COCOAPODS_VERSION_MINOR_JSONModel 12
#define COCOAPODS_VERSION_PATCH_JSONModel 0

